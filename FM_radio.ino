...
/* Constructor for OLED graphic mode */
...
void update_display() {
  char buffer[16];
  sprintf(buffer, "%3d.%1d", current_frequency / 10, current_frequency % 10);
  u8g2.firstPage();
  do {
...
void loop() {
  ...
  while(1) {
    ...
    if (encoder_delta != 0) {
      frequency_adjust(encoder_delta);
      update_display();
      //Serial.println(current_frequency);
      gotoChannel(current_frequency);
    }
    if (storage_time > 0) {
      if (millis() >= storage_time) {
        frequency_store();
      }     
    }
  };
...
/*
  while(1) {
    ...
    sprintf(printBuffer, "Current station: %02d.%01dMHz (RSSI:%ddBuV)", currentChannel / 10, currentChannel % 10, si4703_registers[STATUSRSSI] & 0x00FF);
